...
<head>
    ...
    <style>
        /* No external resources — DomPDF server-side only */
...
<div class="page {{ $i < count($suratJalans) - 1 ? 'page-break' : '' }}">
    ...
    <div class="footer-info">
        <div>NOTE</div><div>:</div><div style="text-transform:capitalize;font-weight:bold">{{ $sj->note ?? '-' }}</div>
        <div>TAKEN BY</div><div>:</div><div style="text-transform:capitalize">{{ $sj->taken_by ?? '-' }}</div>
        <div>VEHICLE NO.</div><div>:</div><div style="text-transform:capitalize">{{ $sj->vehicle_no ?? '-' }}</div>
